#!/usr/bin/env pwsh
# RunCode Windows 构建准备脚本：下载并精简 TDM-GCC 到 src-tauri/resources/mingw64/
#
...
#   2. 用 7z 解压 NSIS 安装器，提取 mingw64 目录
#   3. 精简：删 GDB、mingw32-make、文档
#   4. 复制到 src-tauri/resources/mingw64/
#
...
$dest = "src-tauri/resources/mingw64"
...
# 5. 查找 mingw64 目录
write-host "=== 查找 mingw64 目录 ===" -foregroundcolor yellow
$mingwSrc = get-childitem -path $extractDir -recurse -directory -filter "mingw64" | select-object -first 1
if (-not $mingwSrc) {
    throw "未找到 mingw64 目录，请检查解压结果: $extractDir"
}
write-host "✓ 找到: $($mingwSrc.fullname)"
...
copy-item $mingwSrc.fullname $dest -recurse
